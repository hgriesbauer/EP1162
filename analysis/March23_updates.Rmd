...
```{r Figure 4b graph live BA over time}
...
table4<-
 ...
 mutate(Trt.Unit=factor(paste("Trt:",BA.Target,sep=""))) %>% 
  ...
  summarise(BA=mean(BA,na.rm=TRUE)*20) %>% 
  mutate_at(vars("BA"),round,2) %>% 
  pivot_wider(names_from="Year",values_from="BA") 

# Print to screen
  knitr::kable(table4,format="markdown")
...
**NB**
However, this figure (and subsequent digging) has revealed another issue with the data - some plots were not measured in 1992, which that mean basal area contribution by species will vary between 1992 and 1994.  Note the decrease in Fd basal area per plot between 1992 and 1994.  This is due to different number of plots between 1992 and 1994, not because of Fd mortality (as this figure might imply).  I see a couple of options:

1. Explain this in the report; or
2. Limit analysis for these types of figures to 1994-2019 period.
...
There is only one Fd tree in the entire dataset.
...
dat %>% 
  ...
  group_by(BA.Target,Year,Species,DBH.class) %>% 
    summarise(Num.Trees=mean(Num.Trees)) %>% 
...
*This plot shows the mean number of stems per plot per Treatment Unit/Species/DBH Class/Year*
