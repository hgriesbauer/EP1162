...
To test the hypothesis that tree-level radial growth response over the 1994-2019 period varied with interactions between (i) tree diameter at the time of final treatment (1994); (ii) treatment; and (iii) species, we fit linear mixed effects models with log-transformed 1994-2019 diameter increment as the response variable, and treatment, species and log-transformed 1992 diameter as interacting fixed effects. To account for plot-level productivity differences, we included plot as a random intercept effect.  Models were fit using the 'lme4' package for R [@].  We also fit a similar model with the 1994-2019 diameter increment expressed as a percentage of 1994 diameter.  Plots of residuals and fit lines with orginal data were examined to evaluate model goodness of fit.  Variance explained for each model are reported using the approach described in Nakagawa et al. [-@Nakagawa2013], as implemented in the MuMIn package for R [@barton2009mumin].  Pairwise contrasts were performed using the 'emmeans' package for R [@] and Tukey HSD method of *p* value adjustment was used to control for multiplicity.  We report all significant results at p<0.05.  
...
All treatment units, including the control, increased volume, basal area and stand density over the 1994-2019 period (`r figNums("standPlots",display="cite")` and `r appNums("standPlot.table",display="cite")`).  Mean annual basal area and volume increment peaked during the 1997-2009 period in the two harvested treatments, and peaked in the 1994-1997 period in the control treatment unit (`r figNums("perInc.plot",display="cite")`).  From 1994 to 2019, estimated marginal means of basal area increment in the low RBA unit exceeded marginal means in the high RBA and control unit by an estimated 
...
Across all plots and time periods, smaller trees contributed proportionally more to stand basal area growth than larger trees (`r figNums("gds.plot",display="cite")`).  Over the last two measurement periods, the growth dominance statistic decreased in all plots except one (not shown), indicating that individual tree growth contributions to stand growth became more proportional to tree size over the last 11 growing seasons.    
