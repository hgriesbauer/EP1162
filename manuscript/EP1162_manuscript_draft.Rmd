...
`r figNums(name="sphPCTPlot",caption="Percentage change between 1992 and 2019 in stems per hectare (SPH) by diameter class, faceted by treatment unit and species.   tsphPCTPlotwo measurement periods.  Lowest value is included in each diameter class, highest value is not.")`
...
`r figNums("volPlot.plot",caption="Volume per plot (m³ per hectare) at five measurement years, faceted by treatment unit.")`
...
`r figNums("incPlot",caption="Mean annual increment per plot over four measurement periods.  Top panel row shows basal area (BA), with units of m2 per hecatre.  Middle panel row shows merchantable volume (Vol.Merch), and bottom panel row shows total volume (Vol.Tot).  Volume units are m³ per hectare.")`
